...
STARTER_CATALOG: list[CatalogEntry] = [
    ...
    CatalogEntry(
        dataset_id="DSD_CENSUS_GROUP1_3@DF_B1607",
        theme="Population",
        title="Residents by commune (2021 census)",
        friendly_title="Census population",
        description="Number of residents by commune from the 2021 population census, with the full sex and age breakdown held in the underlying dataset.",
        keywords=["population", "residents", "census", "commune", "municipality", "age", "age structure", "sex"],
        dimensions=["Year", "Commune", "Sex", "Age class"],
        time_periods="2021 census.",
        geography="Commune.",
        recommended=True,
        dashboard="Population",
        notes="Confirmed LUSTAT census table: population by canton and municipality, sex and age. The Commune Portal shows the all-ages, all-sexes total; the full age structure stays in the dataset for the Dataset Explorer.",
        status="confirmed",
        geographic_level="commune",
        geography_column="GEO_LABEL",
        commune_code_column="GEO",
        commune_name_column="GEO_LABEL",
        commune_portal=True,
        default_filters={"SEX": "_T", "AGE": "_T"},
    ),
    CatalogEntry(
        dataset_id="DSD_CENSUS_GROUP7_10@DF_B1625",
        theme="Population",
        title="Foreign residents by commune (2021 census)",
        friendly_title="Foreign residents",
        description="Number of foreign-nationality residents by commune from the 2021 population census.",
        keywords=["foreign residents", "nationality", "citizenship", "census", "commune", "municipality", "foreigners"],
        dimensions=["Year", "Commune", "Citizenship", "Sex"],
        time_periods="2021 census.",
        geography="Commune.",
        recommended=True,
        dashboard="Population",
        notes="Confirmed LUSTAT census table: population by canton and municipality, citizenship and sex. The Commune Portal shows residents of foreign nationality (all sexes); the Luxembourg / EU / non-EU citizenship split stays in the dataset for the Dataset Explorer.",
        status="confirmed",
        geographic_level="commune",
        geography_column="GEO_LABEL",
        commune_code_column="GEO",
        commune_name_column="GEO_LABEL",
        commune_portal=True,
        default_filters={"SEX": "_T", "CITIZEN": "FOR"},
    ),
    CatalogEntry(
    ...
    CatalogEntry(
        ...
        friendly_title="Tourism accommodation activity",
        description="Monthly arrivals and overnight stays by tourist region and accommodation type from STATEC's D5310 workbook.",
        keywords=[
            "tourism", "tourists", "arrivals", "accommodation", "hotels",
            "overnight stays", "nights", "short-term indicators",
            "Luxembourg tourism", "D5310",
        ],
        dimensions=["Month", "Tourist region", "Accommodation type", "Indicator"],
        time_periods="2015 onward in the current workbook.",
        geography="Tourist region.",
        update_frequency="Monthly workbook; check STATEC source for publication timing.",
        recommended=True,
        dashboard="Tourism",
        notes="Reviewed STATEC Excel workbook D5310. LuxStats charts annual totals summed from monthly arrivals and overnight-stay rows.",
        status="confirmed",
